...
# 打印启动命令
STARTUP_LOG="${STARTUP_LOG}application startup command: \tnohup java ${JAVA_OPT} -jar -Dloader.path=${LIB_DIR} ${BASE_PATH}/boot/${APPLICATION_JAR} --spring.config.location=${CONFIG_DIR} > ${LOG_PATH} 2>&1 &\n"
...
java ${JAVA_OPT} \
    -javaagent:${BASE_PATH}/agent/mingsha-javaagent-methodtime-0.0.1-SNAPSHOT.jar \
    -Dmingsha.agent.config.collector.packages="site.mingsha.* "\
